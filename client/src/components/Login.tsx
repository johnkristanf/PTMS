    ...
    <>
      ...
      >
        <div className="flex gap-1">
          {errors.email && (
            <span className="text-red-800 font-bold">Email is required.</span>
          )}
          {errors.password && (
          ...
          {timeLoginTimeout && (
            <span className="text-red-800 font-bold">
              Request took too long to respond due to slow internet
            </span>
          )}
        </div>
...
export function ApplicantAuthentication({
  setRole,
}: {
  setRole: React.Dispatch<React.SetStateAction<string>>;
}) {
...
function ApplicantForgotPasswordForm({
  setAuthType,
}: {
  setAuthType: React.Dispatch<React.SetStateAction<string>>;
}) {
    ...
    <>
      <div className="flex flex-col items-center text-center gap-3">
        ...
        <p className="text-gray-500">
          Please provide the email where you want to get the temporary password
        </p>
      </div>
