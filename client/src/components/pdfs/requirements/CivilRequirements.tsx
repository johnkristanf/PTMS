...
const CivilRequirements = ({
  ...
  const { register, handleSubmit, watch, setValue } =
    useForm<CivilRequirementFormData>();
  ...
  const toggleSelectAll = () => {
    const fields = Object.keys(response?.data || {}) as Array<
      keyof CivilRequirementFormData
    >;
      ...
      <div className="w-full fixed top-24 left-0 h-screen flex justify-center ">
        <div className="flex flex-col items-center h-[80%] py-4 w-[55%] bg-white rounded-md ">
          ...
          <form
            ...
            className="max-w-screen-sm w-full h-[100%] overflow-auto"
          >
            <div className="grid grid-cols-1 gap-3 p-5">
              <h1 className="text-lg font-bold">
                Civil plans and technical specifications containing the
                following:
              </h1>

              <label
                className={`flex items-center font-semibold text-md p-2 rounded-md ${
                  response?.data.Site_Development_Plan
              ...
              <label
                className={`flex items-center font-semibold text-md p-2 rounded-md ${
                  response?.data.Foundation_Plans
                    ? "bg-green-600 text-white"
                    : "bg-gray-200"
                }`}
              >
              ...
              <label
                className={`flex items-center font-semibold text-md p-2 rounded-md ${
                  response?.data.Roof_Floor_Framing_Plans
                    ? "bg-green-600 text-white"
                    : "bg-gray-200"
                }`}
              >
                <input
                  ...
                  disabled={
                    response?.data.Roof_Floor_Framing_Plans ? true : false
                  }
                  className="mr-2 checkbox-large"
                />
                Floor/Roof Framing Plans and Details at scale of not less than
                1:100.
              </label>

              <label
                className={`flex items-center font-semibold text-md p-2 rounded-md ${
                  response?.data.Details_Schedule_Civil_WorkElements
                    ? "bg-green-600 text-white"
                    : "bg-gray-200"
                }`}
              >
                <input
                  ...
                  className="mr-2 checkbox-large"
                />
                Details and Schedules of structural and civil works elements
                including those for deep wells, water reservoir, pipe lines and
                sewer system.
              </label>

              <label
                className={`flex items-center font-semibold text-md p-2 rounded-md ${
                  response?.data.Structural_Analysis_Design
                    ? "bg-green-600 text-white"
                    : "bg-gray-200"
                }`}
              >
                <input
                  ...
                  disabled={
                    response?.data.Structural_Analysis_Design ? true : false
                  }
                  className="mr-2 checkbox-large"
                />
                Structural Analysis and Design for all buildings/structures
                except for one storey and single detached building/structure
                with a total floor area of 20.00 sq. meters or less.
              </label>

              <label
                className={`flex items-center font-semibold text-md p-2 rounded-md ${
                  response?.data.Boring_Load_Test
                    ? "bg-green-600 text-white"
                    : "bg-gray-200"
                }`}
              >
              ...
              <label
                className={`flex items-center font-semibold text-md p-2 rounded-md ${
                  response?.data.Seismic_Analysis
                    ? "bg-green-600 text-white"
                    : "bg-gray-200"
                }`}
              >
